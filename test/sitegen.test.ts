...
function landingArchive(): SiteSubmission[] {
  ...
  return [
    ...
    make("lax-68", [{ conceptId: "Lax68.Planar", title: "Planar graphs" }]),
  ];
}
...
After the formula.`, "");
  ...
  it("renders the index with library rows and a searchable sidebar", async () => {
    ...
    expect(index).toContain('<input id="submissions-search" class="filter-input" type="search" placeholder="Search titles and concepts" aria-controls="submissions-list">');
  ...
  it("shows the examples, the network and the foundations from the archive", async () => {
    ...
    expect(index).toContain('href="lax-68/Lax68.Planar.html" title="Lax68.Planar"');
    expect(index).not.toContain("Lax48.TwinWidth");
