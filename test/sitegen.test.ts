...
\[\frac{a}{b}\]
  ...
  it("emits complete deterministic static output with known MIME types", async () => {
    ...
    expect(css).toContain(".site-nav-getting-started{ display: none; }");
    expect(css).toContain("@media (min-width: 1600px){\n  .site-nav-getting-started{ display: inline; }");
  ...
  it("renders the index with library rows and a searchable sidebar", async () => {
    ...
    expect(index).toContain("30 September 2026 at 10:00 CEST");
    ...
    expect(submissionsIndex).toContain('<a class="site-nav-link" href="../submissions/">Submissions</a>');
    expect(submissionsIndex).not.toContain("site-nav-getting-started");
  ...
  it("shows the examples, the network and the foundations from the archive", async () => {
    ...
    expect(index).toContain('<a class="site-nav-link" href="submissions/">Submissions</a>');
