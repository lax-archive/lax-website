...
After the formula.`, "");
  ...
  it("emits expandable concept closures and proof readiness metadata for deterministic DAGs", async () => {
    ...
    expect(script).toContain("const PROOF_NETWORK_MIN_WIDTH = 720;");
    expect(script).toContain("PROOF_NETWORK_MIN_WIDTH,\n      container.clientWidth,");
    expect(script).toContain("const PROOF_SELECTION_SCALE = 1.28;");
    expect(script).toContain("const GRAPH_ZOOM_MIN = 0.25;");
    expect(script).toContain("const GRAPH_SCROLL_DURATION = 900;");
    expect(script).toContain("setProofMagnification(context, true)");
    expect(script).toContain("setProofMagnification(context, false)");
    expect(script).toContain("installGraphZoomControls");
    expect(script).toContain("animateGraphScroll");
    expect(script).toContain("figure.classList.contains('graph-expanded')");
    expect(script).toContain("placeDetailPanel");
    expect(script).toContain("DETAIL_PANEL_MIN_OUTSIDE_WIDTH");
    expect(script).toContain("classList.add('graph-detail-right')");
    expect(script).not.toContain("classList.toggle('graph-detail-left'");
    expect(script).toContain("context.panel.contains(target)");
    expect(script).toContain(".graph-edge-route.is-interactive");
    ...
    expect(css).toContain(".graph-detail-panel.graph-detail-outside.graph-detail-right{");
    ...
    expect(css).toContain(".graph-expanded .graph-zoom-controls{ display: inline-flex; }");
    expect(css).toContain(".graph-zoom-controls [data-graph-zoom-reset]");
