...
\[\frac{a}{b}\]
  ...
  it("renders the submission page: paper masthead, compact grids, citation, graph data", async () => {
    ...
    expect(html).toMatch(/<span class="proof-flow">assumptions <svg class="legend-assumptions"[^>]*><defs>.*?<\/defs>(<path[^>]*\/>){3}<\/svg><i class="legend-proof-chip" aria-hidden="true">⊢<\/i><svg class="legend-arrow legend-flow-arrow"[^>]*><defs>.*?<\/defs><path[^>]*\/><\/svg> conclusion<\/span>/);
