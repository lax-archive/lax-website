...
export function pageReactions(pathname: string, options: PageReviewOptions = {}): string {
  ...
  const flaggedNote = conceptURLs.length
    ...
    : "";
  return `<section class="page-reactions" aria-label="Community review" data-reactions-host="${attr(REMARK42_URL)}" data-reactions-url="${attr(threadUrl)}" data-review-kind="${kind}" data-source-lines="${sourceLines}"${conceptData}>
