...
export function proofPage(ctx: PageContext, located: LocatedProof): string {
  ...
  const archivedSource = submission.integrity?.proofFiles[proof.id];
  const integrityNote = archivedSource
    ? `<p class="honesty-note">Archived proof source SHA-256 <code>${esc(archivedSource.sha256)}</code> · validation capture <code>sha256:${esc(submission.integrity!.captureDigest)}</code> · commit <code>${esc(submission.integrity!.sourceCommit)}</code></p>`
    : "";
