...
import { isDiscoverableSubmission } from "../model.js";
import type { PageContext } from "./shared.js";

/** Direct-only public activity stream. Deliberately omitted from site navigation. */
export function allCommentsPage(ctx: PageContext): string {
  const listed = ctx.model.submissions
    .filter(isDiscoverableSubmission)
    .map((submission) => submission.record.id.toLowerCase());
...
<section class="activity-stream" id="all-comments" aria-labelledby="activity-heading" aria-busy="true" data-remark42-host="${attr(REMARK42_URL)}" data-remark42-site="${attr(REMARK42_SITE_ID)}" data-identity-url="${attr(REMARK42_IDENTITY_URL)}" data-listed-submissions="${attr(JSON.stringify(listed))}">
