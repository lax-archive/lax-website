...
export function allCommentsPage(ctx: PageContext): string {
  const unlisted = ctx.model.submissions
    .filter((submission) => submission.output?.manifest.unlisted === true)
...
<section class="activity-stream" id="all-comments" aria-labelledby="activity-heading" aria-busy="true" data-remark42-host="${attr(REMARK42_URL)}" data-remark42-site="${attr(REMARK42_SITE_ID)}" data-identity-url="${attr(REMARK42_IDENTITY_URL)}" data-unlisted-submissions="${attr(JSON.stringify(unlisted))}">
