...
import { randomSubmissionView, type PageContext } from "./shared.js";

/** Render a repository-owned editorial Markdown page. */
export function contentPage(
  { model, markdown }: PageContext,
  name: string,
  title: string,
): string {
  const source = contentMarkdown(`${name}.md`)
    .replace(/^#\s+.*(?:\r?\n)+/, "");
  const sidebar = `${randomSubmissionView(model, markdown, "")}
<a class="sidebar-back" href="index.html"><span class="sidebar-back-arrow" aria-hidden="true">←</span>Archive</a>
<ul id="entry-list">
